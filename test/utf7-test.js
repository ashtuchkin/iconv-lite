...
describe("UTF-7 codec", function () {
    it("encodes correctly", function () {
        ...
        assert.equal(
            iconv.encode("'\u4F60\u597D' heißt \"Hallo\"", "utf-7").toString(),
            "'+T2BZfQ-' hei+AN8-t +ACI-Hallo+ACI-"
        );

        // The plus sign is represented as +-.
        assert.equal(
            iconv.encode("Hot + Spicy + Fruity", "utf-7").toString(),
            "Hot +- Spicy +- Fruity"
        );

        // Slashes in the beginning.
        assert.equal(
            iconv.encode("\uffff\uedca\u9876\u5432\u1fed", "utf-7").toString(),
            "+///typh2VDIf7Q-"
        );
    ...
    it("decodes correctly", function () {
        ...
        assert.equal(
            iconv.decode(Buffer.from("Hi+ACA-Mom+ACA--+Jjo--+ACE-"), "utf-7"),
            "Hi Mom -\u263A-!"
        );
        assert.equal(iconv.decode(Buffer.from("Item 3 is +AKM-1."), "utf-7"), "Item 3 is \u00A31.");
        assert.equal(
            iconv.decode(Buffer.from("Item+ACA-3+ACA-is+ACAAow-1."), "utf-7"),
            "Item 3 is \u00A31."
        );

        // Custom examples that contain more than one mode shift.
        assert.equal(
            iconv.decode(Buffer.from("Jyv+AOQ-skyl+AOQ-"), "utf-7"),
            "Jyv\u00E4skyl\u00E4"
        );
        assert.equal(iconv.decode(Buffer.from("Jyv+AOQ-skyl+AOQ"), "utf-7"), "Jyv\u00E4skyl\u00E4");
        assert.equal(
            iconv.decode(Buffer.from("'+T2BZfQ-' hei+AN8-t \"Hallo\""), "utf-7"),
            "'\u4F60\u597D' heißt \"Hallo\""
        );
        assert.equal(
            iconv.decode(Buffer.from("'+T2BZfQ' hei+AN8-t \"Hallo\""), "utf-7"),
        ...
        assert.equal(
            iconv.decode(Buffer.from("'+T2BZfQ-'+ACA-hei+AN8-t+ACAAIg-Hallo+ACI-"), "utf-7"),
            "'\u4F60\u597D' heißt \"Hallo\""
        );
        assert.equal(
            iconv.decode(Buffer.from("'+T2BZfQ-'+ACA-hei+AN8-t+ACAAIg-Hallo+ACI"), "utf-7"),
        ...
        assert.equal(
            iconv.decode(Buffer.from("Hot +- Spicy +- Fruity"), "utf-7"),
            "Hot + Spicy + Fruity"
        );
        assert.equal(
            iconv.decode(Buffer.from("Hot+ACAAKwAg-Spicy+ACAAKwAg-Fruity"), "utf-7"),
            "Hot + Spicy + Fruity"
        );

        // Slashes in the beginning.
        assert.equal(
            iconv.decode(Buffer.from("+///typh2VDIf7Q-"), "utf-7"),
            "\uffff\uedca\u9876\u5432\u1fed"
        );
        assert.equal(
            iconv.decode(Buffer.from("+///typh2VDIf7Q"), "utf-7"),
        ...
        // + sign around non-ASCII chars
        assert.equal(
            iconv.decode(Buffer.from("+AOQ-+-+AOQ-+-+AOQ-"), "utf-7"),
            "\u00E4+\u00E4+\u00E4"
        );
        //assert.equal(iconv.decode(Buffer.from('+AOQ++AOQ+-+AOQ'), 'utf-7'), '\u00E4+\u00E4+\u00E4');
        assert.equal(
            iconv.decode(Buffer.from("+AOQAKwDkACsA5A-"), "utf-7"),
            "\u00E4+\u00E4+\u00E4"
        );
...
describe("UTF-7-IMAP codec", function () {
    it("encodes correctly", function () {
        ...
        assert.equal(
            iconv.encode("Item 3 is \u00A31.", "utf-7-imap").toString(),
            "Item 3 is &AKM-1."
        );

        // Custom examples that contain more than one mode shift.
        assert.equal(
            iconv.encode("Jyv\u00E4skyl\u00E4", "utf-7-imap").toString(),
            "Jyv&AOQ-skyl&AOQ-"
        );
        assert.equal(
            iconv.encode("'\u4F60\u597D' heißt \"Hallo\"", "utf-7-imap").toString(),
            "'&T2BZfQ-' hei&AN8-t \"Hallo\""
        );

        // The ampersand sign is represented as &-.
        assert.equal(
            iconv.encode("Hot & Spicy & Fruity", "utf-7-imap").toString(),
            "Hot &- Spicy &- Fruity"
        );

        // Slashes are converted to commas.
        assert.equal(
            iconv.encode("\uffff\uedca\u9876\u5432\u1fed", "utf-7-imap").toString(),
            "&,,,typh2VDIf7Q-"
        );

        // & sign around non-ASCII chars
        assert.equal(
            iconv.encode("\u00E4&\u00E4&\u00E4", "utf-7-imap").toString(),
            "&AOQ-&-&AOQ-&-&AOQ-"
        );
    });
    ...
    it("decodes correctly", function () {
        ...
        assert.equal(
            iconv.decode(Buffer.from("Hi Mom -&Jjo--!"), "utf-7-imap"),
            "Hi Mom -\u263A-!"
        );
        assert.equal(
            iconv.decode(Buffer.from("Item 3 is &AKM-1."), "utf-7-imap"),
            "Item 3 is \u00A31."
        );

        // Custom examples that contain more than one mode shift.
        assert.equal(
            iconv.decode(Buffer.from("Jyv&AOQ-skyl&AOQ-"), "utf-7-imap"),
            "Jyv\u00E4skyl\u00E4"
        );
        assert.equal(
            iconv.decode(Buffer.from("'&T2BZfQ-' hei&AN8-t \"Hallo\""), "utf-7-imap"),
            "'\u4F60\u597D' heißt \"Hallo\""
        );

        // The ampersand sign is represented by &-.
        assert.equal(
            iconv.decode(Buffer.from("Hot &- Spicy &- Fruity"), "utf-7-imap"),
            "Hot & Spicy & Fruity"
        );

        // Slashes are converted to commas.
        assert.equal(
            iconv.decode(Buffer.from("&,,,typh2VDIf7Q-"), "utf-7-imap"),
            "\uffff\uedca\u9876\u5432\u1fed"
        );

        // & sign around non-ASCII chars
        assert.equal(
            iconv.decode(Buffer.from("&AOQ-&-&AOQ-&-&AOQ-"), "utf-7-imap"),
            "\u00E4&\u00E4&\u00E4"
        );
    });
});
